...
deploy() {
  ...
  # copy creds + persona into the VM first
  sshpass -p "$PW" scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -r /tmp/creds /tmp/personas ${user}@${ip}:/tmp/ 2>&1 | tail -1

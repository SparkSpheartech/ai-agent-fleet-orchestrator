...
setup() {
  ...
  sshpass -p "$PW" scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null /tmp/hermes.service /tmp/hermes-gateway.service ${user}@${ip}:/tmp/ 2>&1 | tail -1
  sshpass -p "$PW" scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -r /tmp/ocr_skill_pkg/ocr ${user}@${ip}:/tmp/ocr 2>&1 | tail -1

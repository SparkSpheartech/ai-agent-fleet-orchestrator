...
setup() {
  ...
  # copy OCR skill + service files into VM
  sshpass -p "$PW" scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -r /tmp/ocr_skill_pkg/ocr ${user}@${ip}:/tmp/ocr_skill_pkg_ocr 2>&1 | tail -1
  sshpass -p "$PW" ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null ${user}@${ip} bash -s <<EOF
set -e
mkdir -p ~/.hermes/skills/ocr
cp /tmp/ocr_skill_pkg_ocr/ocr.py ~/.hermes/skills/ocr/ocr.py
cp /tmp/ocr_skill_pkg_ocr/SKILL.md ~/.hermes/skills/ocr/SKILL.md
# install service files (substitute user)
